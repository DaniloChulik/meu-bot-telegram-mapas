...
# Sai se os placeholders não foram substituídos (ajuste os placeholders se os seus forem diferentes)
...
if not API_KEY_GOOGLE_MAPS_DIRETO or API_KEY_GOOGLE_MAPS_DIRETO == "SUA_CHAVE_PLACEHOLDER_AQUI":
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if directions_result and len(directions_result) > 0:
            ...
            print(f"handle_message: Nenhuma rota encontrada pela API. Status: {status_api}")
        ...
        print(f"Erro HTTP da API Google Maps: {http_err}, Conteúdo: {error_content}")
...
if __name__ == '__main__':
    ...
    try:
        application = ApplicationBuilder().token(TOKEN_TELEGRAM_DIRETO).build() # USA TOKEN_TELEGRAM_DIRETO
